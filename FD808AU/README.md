...
* /Common - Contains common code split by functionality which should rarely need to be modified. 
	* `Audio Unit/FD808AUAudioUnit.swift` - A subclass of AUAudioUnit, this is the actual Audio Unit implementation. You may in advanced cases need to change this file to add additional functionality from AUAudioUnit.  
* /Parameters
	* `FD808AUParameterAddresses.h` - A pure `C` enum containing parameter addresses used by Swift and C++ to reference parameters.
...
* /DSP
	* `FD808AUDSPKernel.hpp` - A pure C++ class to handle the real-time aspects of the Audio Unit Extension. DSP and processing should be done here. Note: Be aware of the constraints of real-time audio processing. 
* /UI
	* `FD808AUMainView.swift` - SwiftUI based main view, add your SwiftUI views and controls here.

## Adding a parameter
1. Add a new parameter address to the `FD808AUParameterAddress` enum in `FD808AUParameterAddresses.h` 


Example:

```c
typedef NS_ENUM(AUParameterAddress, FD808AUParameterAddress) {
	sendNote = 0,
	....
	attack
```
...
3. In order to manipulate the DSP side of the Audio Unit we must handle changes to our new parameter in `FD808AUDSPKernel.hpp`. In the `setParameter` and `getParameter` methods add a case for the new parameter address.
...
```cpp
	void setParameter(AUParameterAddress address, AUValue value) {
		switch (address) {
			....
			case FD808AUExtensionParameterAddress:: attack:
				mAttack = value;
				break;			
			...
	}
