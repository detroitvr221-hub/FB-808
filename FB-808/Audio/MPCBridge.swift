...
// MARK: - MPC Coach strip (Settings → Pads → "MPC Coach")

/// The in-place consumer of `AppSettings.mpcCoach` (the toggle used to be persisted but read nowhere —
/// SETTINGS_AUDIT_2026-09-16 #1). When on, a mode shows the MPC equivalent of its controls as a compact
/// chip row right under its header, pulled from the same table the MPC Bridge sheet renders.
struct MPCCoachStrip: View {
    @EnvironmentObject var settings: AppSettings
    let section: String            // an `MPCBridge.sections` title, e.g. "Playing the Pads"
    var body: some View {
        if settings.mpcCoach, let sec = MPCBridge.sections.first(where: { $0.title == section }) {
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 6) {
                    Image(systemName: "book.fill").font(.system(size: 10, weight: .bold)).foregroundStyle(settings.accent)
                        .accessibilityHidden(true)
                    ForEach(Array(sec.rows.enumerated()), id: \.offset) { _, r in
                        HStack(spacing: 4) {
                            Text(r.fd).font(FDFont.ui(10.5, .semibold)).foregroundStyle(settings.inkDim)
                            Image(systemName: "arrow.right").font(.system(size: 8, weight: .bold)).foregroundStyle(settings.inkFaint)
                            Text(r.mpc).font(FDFont.mono(10, .bold)).foregroundStyle(settings.accent)
                        }
                        .lineLimit(1)
                        .padding(.horizontal, 8).frame(height: 24)
                        .background(Capsule().fill(settings.accent.opacity(0.1)))
                        .overlay(Capsule().stroke(settings.accent.opacity(0.3), lineWidth: 1))
                        .accessibilityElement(children: .ignore)
                        .accessibilityLabel(Text("\(r.fd), on an MPC: \(r.mpc)"))
                    }
                }
            }
            .accessibilityLabel(Text("MPC Coach"))
        }
    }
}
