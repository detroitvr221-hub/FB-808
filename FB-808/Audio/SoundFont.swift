...
enum SoundFont {
    ...
    static func load(_ data: Data) -> SFInstrument? {
        ...
        for bag in firstBag..<bagEnd {
            ...
            // Pitch generators (finding 45): coarseTune is in semitones, fineTune in cents. Both were
            // ignored entirely, and the header's own chPitchCorrection was parsed and then dropped on the
            // way into the engine, so an imported zone played detuned by its built-in correction.
            var coarseTune = 0, fineTune = 0
            for g in gLo..<min(max(gLo, gHi), genCount) {
                ...
                case 51: coarseTune = Int(Int16(bitPattern: UInt16(truncatingIfNeeded: amt)))   // semitones
                case 52: fineTune = Int(Int16(bitPattern: UInt16(truncatingIfNeeded: amt)))     // cents
            ...
            regions.append(SFRegion(loKey: loKey, hiKey: hiKey,
                                    ...
                                    tuneCents: sh.corr + coarseTune * 100 + fineTune,
