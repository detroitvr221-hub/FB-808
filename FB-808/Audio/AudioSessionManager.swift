...
final class AudioSessionManager: ObservableObject {
    ...
    /// The last `setActive` failure, in words — `AudioEngine.start` reports it in the UI banner instead of the
    /// old silent `try?` (SYSTEMS_GAP_AUDIT #lifecycle-8).
    ...
    var targetFrames: Int {
        ...
        if routeClass == .bluetooth { return 1024 }
        return DeviceTier.current.automaticBufferFrames
    }
    ...
    private func activate(_ s: AVAudioSession) {
        do { try s.setActive(true); lastActivationError = nil }
        catch { lastActivationError = "Audio couldn't start: \(error.localizedDescription)" }
    }
