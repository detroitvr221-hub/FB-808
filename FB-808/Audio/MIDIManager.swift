...
final class MIDIInputFIFO: @unchecked Sendable {
    ...
    private func push(_ s: UInt8, _ d1: UInt8, _ d2: UInt8) {
        if count == cap { overflowed = true; tail = (tail + 1) % cap; count -= 1 }   // drop oldest
    ...
    func drain() -> [Raw] {
        ...
        if overflowed {
            overflowed = false; tail = head; count = 0
            return [Raw(status: 0xB0, d1: 120, d2: 0)] // a lost note-off must never leave a sustained voice
        }
    ...
    func resetParser() { os_unfair_lock_lock(&lock); runningStatus = 0; dataIdx = 0; inSysex = false; head = 0; tail = 0; count = 0; overflowed = false; os_unfair_lock_unlock(&lock) }
}
...
final class MIDIManager: ObservableObject {
    ...
    var onNoteOn: ((Int, Double, Int) -> Void)?               // (midi note, velocity 0…1) — melodic
    var onNoteOff: ((Int, Int) -> Void)?                      // (midi note) — melodic
    var onCC: ((Int, Int, Int) -> Void)?                 // (cc, value, channel)
    var onPanic: (() -> Void)?                           // all-notes-off (CC123)

    private var sustainedChannels: Set<UInt8> = []
    private var deferredNoteOffs: [UInt8: Set<Int>] = [:]
    ...
    private func panic() {
        sustainedChannels.removeAll()
        deferredNoteOffs.removeAll()
        onPanic?()
    }

    func dispatch(_ e: MIDIInputFIFO.Raw) {
            ...
            case 0x90 where e.d2 > 0:                    // note on (velocity > 0)
                ...
                if chan == Self.drumChannel { dispatchPad(note: Int(e.d1), vel: vel) }
                    ...
                    deferredNoteOffs[chan]?.remove(Int(e.d1))
                    onNoteOn?(Int(e.d1), vel, Int(chan)); lastEvent = "Note \(e.d1) v\(e.d2)"
                }
            case 0x80, 0x90:                             // note off (0x80, or note-on velocity 0)
                if chan != Self.drumChannel {
                    if sustainedChannels.contains(chan) { deferredNoteOffs[chan, default: []].insert(Int(e.d1)) }
                    else { onNoteOff?(Int(e.d1), Int(chan)) }
                }   // pads are one-shots → ignore note-off
            case 0xB0:                                   // control change
                if e.d1 == 123 || e.d1 == 120 { panic(); lastEvent = "All notes off" }   // 123 all-off / 120 all-sound-off
                else if e.d1 == 64 {
                    if e.d2 >= 64 { sustainedChannels.insert(chan) }
                    else {
                        sustainedChannels.remove(chan)
                        for note in deferredNoteOffs.removeValue(forKey: chan) ?? [] { onNoteOff?(note, Int(chan)) }
                    }
                    lastEvent = e.d2 >= 64 ? "Sustain on" : "Sustain off"
                }
