...
struct SettingsSheet: View {
    // NB: `engine` and `midi` are deliberately NOT observed here — `engine.diag` publishes at 5 Hz and
    // used to re-render the whole sheet (every card, the achievements grid, the share report) five times
    // a second. The two rows that need them are their own views below (SETTINGS_AUDIT_2026-09-16 #7).
    @EnvironmentObject var settings: AppSettings
    @EnvironmentObject var progress: ProgressStore
    @EnvironmentObject var link: LinkClock
    @Environment(\.dismiss) private var dismiss
    @State private var showMPCBridge = false
    @State private var showLinkSettings = false
    ...
    var body: some View {
        NavigationStack {
            ...
            .sheet(isPresented: $showLinkSettings) { LinkSettingsView(clock: link).ignoresSafeArea() }
            ...
            .confirmationDialog("Reset all settings?", isPresented: $showResetConfirm, titleVisibility: .visible) {
                ...
                Text("Look, workspace, pad, and audio preferences go back to their defaults (the sample rate takes effect after you quit and reopen). Your saved kits, synth patches, projects, Ableton Link setting, and daily XP goal are not touched.")
            }
        }
    }
    ...
    private var lookCard: some View {
        card("Look", icon: "paintbrush.fill") {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                // Label the Slider itself: wrapping it in a combined element could drop the adjustable
                // trait, leaving VoiceOver able to read the value but not change it (SETTINGS_AUDIT #6).
        ...
        card("Pads", icon: "square.grid.4x3.fill") {
            ...
            Toggle(isOn: $settings.mpcCoach) {
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text("Shows the MPC equivalent of each control under the Pads, Pad Editor and Sample headers").font(FDFont.ui(11.5)).foregroundStyle(settings.inkFaint)
                        .fixedSize(horizontal: false, vertical: true)
                }
            }.tint(settings.accent)
            Toggle(isOn: $settings.padTouchVelocity) {
                VStack(alignment: .leading, spacing: 2) {
                    Text("Touch-sensitive pads").font(FDFont.ui(15, .medium)).foregroundStyle(settings.ink)
                    Text("Flatter, harder hits play louder (uses finger contact size). Off = every hit at the same level.")
    ...
    /// Ableton Link (A17). Every Link path is gated on LinkKit's own "enabled" preference, which only its
    /// settings panel used to write — with no UI here the feature was unreachable dead code. (#20)
    private var syncCard: some View {
        card("Sync", icon: "link") {
            Toggle(isOn: Binding(get: { link.enabled }, set: { link.setEnabled($0) })) {
                VStack(alignment: .leading, spacing: 2) {
                    Text("Ableton Link").font(FDFont.ui(15, .medium)).foregroundStyle(settings.ink)
                    Text("Play in time with other Link-enabled apps on this network.").font(FDFont.ui(11.5)).foregroundStyle(settings.inkFaint)
                }
            }.tint(settings.accent)
            if link.enabled {
                diagRow("Session tempo", String(format: "%.1f BPM", link.tempo), settings.ink)
                diagRow("Peers", link.connected ? "Connected" : "Searching…", link.connected ? settings.theme.good : settings.inkDim)
            }
            Button { showLinkSettings = true } label: {
                Text("Link settings…").font(FDFont.ui(13, .semibold)).foregroundStyle(settings.accent)
                    .frame(maxWidth: .infinity).frame(height: 34)
                    .background(RoundedRectangle(cornerRadius: 9).fill(settings.accent.opacity(0.12)))
            }.buttonStyle(.plain)
            footnote("Link's session tempo drives the grid while it is on (it never overwrites the beat's saved BPM), and the downbeat is eased toward the session's bar phase.")
        }
    }
    ...
    private var audioCard: some View {
        card("Audio", icon: "waveform") {
            ...
            footnote("Changing the buffer restarts the audio engine — a brief gap if the beat is playing.")
            radioRow(title: "Max voices · polyphony",
                     options: [("32", "32"), ("64", "64"), ("96", "96"), ("128", "128")],
                     selected: "\(min(settings.polyphony, DeviceTier.current.maximumVoices))",
                     disabled: Set([32, 64, 96, 128].filter { $0 > DeviceTier.current.maximumVoices }.map(String.init))) { v in settings.polyphony = Int(v) ?? 64 }
            radioRow(title: "Sample rate · applies on restart",
                     ...
                     selected: "\(Int(DeviceTier.current.sampleRate(settings.sampleRate)))",
                     disabled: DeviceTier.current == .high ? [] : ["88200", "96000"]) { v in settings.sampleRate = Double(v) ?? AudioDefaults.sampleRate }
            if DeviceTier.current != .high {
                footnote("This iPad uses up to \(DeviceTier.current.maximumVoices) voices at 48 kHz to leave more headroom for effects and sampling. Auto latency is recommended; large sessions still need device testing.")
            } else {
                footnote("Higher rates reduce aliasing for cleaner synths. A new rate takes effect after you quit and reopen FD-808.")
            }
            RecordingInputRow()
            ...
            Toggle(isOn: $settings.haptics) {
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text("A haptic pulse on each beat while playing, and a light tap on every pad hit — feel the rhythm as well as hear it.").font(FDFont.ui(11.5)).foregroundStyle(settings.inkFaint)
                }
            }.tint(settings.accent)
        }
    }
...
// Its own view so only this row re-renders when the engine publishes (#7).
private struct RecordingInputRow: View {
    @EnvironmentObject var settings: AppSettings
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 9) {
            ...
            AudioInputPicker(prepare: { engine.prepareInputSelection() },
                             onDismiss: { engine.inputSelectionDismissed() }) {
                HStack(spacing: 10) {
            ...
            Text("Record through a built-in mic, headset, or a USB-C / Bluetooth audio interface (e.g. Focusrite). The system remembers your choice per app. Opening the picker asks for microphone access the first time.")
                .font(FDFont.ui(11.5)).foregroundStyle(settings.inkFaint).fixedSize(horizontal: false, vertical: true)
        }
    }
}
...
extension SettingsSheet {
    fileprivate var qualityCardBody: some View {
    ...
    private var diagnosticsCard: some View {
        card("Diagnostics", icon: "stethoscope") {
            DiagnosticsPanel()   // observes the engine on its own so the 5 Hz telemetry doesn't redraw the sheet (#7)
        }
    }
...
// Live engine telemetry (Phase 0). engine.diag refreshes ~5 Hz; only THIS view re-renders for it.
private struct DiagnosticsPanel: View {
    ...
    var body: some View {
        ...
        return VStack(spacing: 7) {
            ...
            // The engine's CAPPED limit (device tier + thermal), not the raw Settings value — a low-tier
            // iPad with 128 stored used to read "0 / 128" while rendering with 32 (SETTINGS_AUDIT #5).
            diagRow("Active voices", "\(d.activeVoices) / \(engine.effectivePolyphony)", settings.ink)
...
extension SettingsSheet {
    // The sync card's tempo/peer rows reuse the same row style.
    fileprivate func diagRow(_ label: String, _ value: String, _ color: Color) -> some View {
        HStack {
            Text(label).font(FDFont.ui(12.5)).foregroundStyle(settings.inkDim)
            Spacer()
            Text(value).font(FDFont.mono(12.5, .bold)).foregroundStyle(color)
