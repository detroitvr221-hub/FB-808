...
struct SampleModeView: View {
    ...
    @State private var stemGeneration = UUID()
    ...
    @State private var waveSamples: [Float] = [] // raw edited buffer for the CPU canvas (shares storage with gpuBuf's array — no copy)
    ...
    private func refreshGPUWave() {
        gpuSig = sampleSig
        guard project.sample != nil else { gpuBuf = nil; waveSamples = []; return }
        let data = engine.currentSampleData()
        waveSamples = data                                  // CPU fallback canvas reads this at pixel resolution
    ...
    var body: some View {
        VStack(spacing: 12) {
            TransportBar()
            StageSplit(sideWidth: 264, spacing: 18, minStage: 460) { _ in
                main
            } side: {
                side
            }
    ...
    private var main: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            MPCCoachStrip(section: "Sampling & Chopping")   // Settings → MPC Coach
            waveBox
            sourceRow
    ...
    @ViewBuilder private func sampleWaveLayer(_ s: SampleState, size: CGSize) -> some View {
        ...
        } else if let buf = gpuBuf, buf.count > 1, Waveform.isRenderAvailable {
    ...
    private func waveCanvas(_ s: SampleState, size: CGSize) -> some View {
        Canvas { ctx, sz in
            ...
            let raw = waveSamples
            let n = raw.count
            guard n > 0 || !s.wave.isEmpty else { return }
            // One peak per drawn pixel over the visible zoom window. Indexing the fixed 1000-bucket
            // s.wave here collapsed to a handful of bars (or a slab) at any zoom — waveform #82.
            for x in stride(from: 0.0, to: sz.width, by: 1) {
                let amp: Double
                if n > 0 {
                    let f0 = waveFrac(x, sz.width), f1 = waveFrac(x + 1, sz.width)
                    let a = max(0, min(n - 1, Int(min(f0, f1) * Double(n))))
                    let b = max(a + 1, min(n, Int(max(f0, f1) * Double(n)) + 1))
                    var peak = 0.0
                    var i = a
                    while i < b { peak = max(peak, abs(Double(raw[i]))); i += 1 }
                    amp = peak
                } else {
                    let nb = s.wave.count
                    let f = waveFrac(x, sz.width)
                    amp = s.wave[max(0, min(nb - 1, Int(f * Double(nb))))]
                }
    ...
    private func snapTrim(_ side: String) {
        ...
        guard let snapped = Self.snappedTrim(side: side, snapped: f, current: s.trim) else { return }
        s.trim[i] = snapped
        project.sample = s
    }

    /// The trim edge a zero-crossing snap should land on, or nil when the snap would not move it. The drag
    /// grip uses `minimumDistance: 0`, so a plain tap runs the snap: an unconditional write there re-dirtied
    /// the project and pushed an undo step for no change. `snapped` is the raw zero-crossing fraction;
    /// `current` is `SampleState.trim` (start, end).
    nonisolated static func snappedTrim(side: String, snapped: Double, current: [Double]) -> Double? {
        let i = side == "l" ? 0 : 1
        let clamped = i == 0 ? max(0, min(snapped, current[1] - 0.02))
                             : min(1, max(snapped, current[0] + 0.02))
        return abs(clamped - current[i]) > 1e-9 ? clamped : nil
    }

    /// A trim-handle drag must open ONE undo step (and mark the project dirty) before its first write, so
    /// the trim is undoable and autosave/recovery capture it. `orig` is the in-flight drag origin for that
    /// side: nil means no drag is open yet, i.e. this write starts one (#TRIM-UNDO).
    nonisolated static func trimNeedsCheckpoint(orig: Double?) -> Bool {
        orig == nil
    }

    /// Whether an in-flight trim frame should checkpoint and write at all. A plain tap on the handle
    /// (`minimumDistance: 0`) delivers a zero-translation frame; that must do NOTHING — no checkpoint,
    /// no dirty flag, no undo entry — which is the defect of finding 40. Once the drag is open every
    /// frame proceeds, because a later frame can legitimately move the edge back and still needs to
    /// write; double-checkpointing is prevented separately by `trimNeedsCheckpoint`. (#TRIM-UNDO)
    nonisolated static func trimFrameShouldApply(moved: Bool, dragOpen: Bool) -> Bool {
        moved || dragOpen
    }
    ...
    private func handle(side: String, f: Double, width: Double) -> some View {
        ...
        .gesture(DragGesture(minimumDistance: 0, coordinateSpace: .named("wave"))
            .onChanged { v in
                ...
                var next = s
                if side == "l" { next.trim[0] = max(0, min(nf, s.trim[1] - 0.02)) }
                else { next.trim[1] = min(1, max(nf, s.trim[0] + 0.02)) }
                let moved = side == "l" ? next.trim[0] != s.trim[0] : next.trim[1] != s.trim[1]
                // The gesture uses minimumDistance: 0, so a plain TAP delivers a zero-translation frame.
                // Opening an undo step there would mark the project dirty and burn redo history without
                // changing anything — the defect of finding 40, which checking in on the drag's first
                // frame re-introduced. Only proceed once the trim actually moves; `trimDragOrig` still
                // marks the drag open so later frames never double-checkpoint. (#TRIM-UNDO)
                guard Self.trimFrameShouldApply(moved: moved, dragOpen: trimDragOrig[side] != nil) else { return }
                if Self.trimNeedsCheckpoint(orig: trimDragOrig[side]) {
                    _ = project.checkpoint("trim", coalesce: true)   // one undo step per drag + dirty (#TRIM-UNDO)
                }
        ...
        .accessibilityAdjustableAction { dir in   // A1 — the one adjustable-action gap the VO pass missed
            guard var s = project.sample else { return }
            _ = project.checkpoint("trimVO", coalesce: true)   // undoable + dirty; merged across VO steps
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 10) {
                ...
                AudioInputPicker(prepare: { engine.prepareInputSelection() },
                                 onDismiss: { engine.inputSelectionDismissed() }) {   // put the session back (#lifecycle-7)
                    HStack(spacing: 8) {
    ...
    /// StageSplit owns the scrolling (see body) — this is plain content.
    private var side: some View {
        ...
        Group { if has { editPanels } else { emptySidePanel } }
    }
                ...
                PanelCard(title: "Stem Split") {
                        ...
                        if stemBusy {
                            HStack(spacing: 8) {
                                ...
                                Button { cancelStemWork(); flash("Stem split cancelled") } label: {   // a slow model download shouldn't be a wall (S1)
    ...
    private func detectTempoKey() {
        ...
        let destination = project.operationDestination()   // commit only into THIS project/sample (#cross-1)
        Task.detached(priority: .userInitiated) {
            let bpm = SynthCore.detectTempo(src.data, sr: src.sr)
            let key = SynthCore.detectKey(src.data, sr: src.sr)
            await MainActor.run {
                analysisBusy = false
                guard destination.matches(project) else { flash("Project changed — tempo/key result discarded"); return }
                var parts: [String] = []
    ...
    private func cancelStemWork() {
        stemTask?.cancel(); stemTask = nil; stemGeneration = UUID()
        stemBusy = false; stemProgress = nil
    }
    private func acceptStemResult(_ destination: ProjectOperationDestination, generation: UUID) -> Bool {
        guard stemGeneration == generation else { return false }
        guard !Task.isCancelled, destination.matches(project) else {
            stemBusy = false; stemTask = nil; stemProgress = nil
            flash(Task.isCancelled ? "Stem split cancelled" : "Project or sample changed — stem split discarded")
            return false
        }
        return true
    }
    private func splitStems() {
        ...
        let generation = UUID(); stemGeneration = generation
        ...
        stemTask = Task.detached(priority: .userInitiated) {
            ...
            await MainActor.run {
                guard acceptStemResult(destination, generation: generation) else { return }
                ...
                project.setPadSamples([(drumID, p, "Drums"), (melID, h, "Melody")], bank: destination.bank)   // one undo step, this bank only (F0)
    ...
    private func splitFourStems() {
        ...
        stemTask = Task.detached(priority: .userInitiated) {
            ...
            if Task.isCancelled {
                await MainActor.run {
                guard acceptStemResult(destination, generation: generation) else { return }
                    stemBusy = false; stemTask = nil; flash("Stem split cancelled")
                }
                return
            }
            let shouldSeparate = await MainActor.run {
                    guard acceptStemResult(destination, generation: generation) else { return false }
                flash("Separating 4 stems… this can take a moment")
                return true
            }
            guard shouldSeparate else { return }
            let stems = FourStemSeparator.separate(src.data, engineSR: src.sr) { p in
                Task { @MainActor in if stemBusy && stemGeneration == generation && destination.matches(project) { stemProgress = p } }
            }
            await MainActor.run {
                guard acceptStemResult(destination, generation: generation) else { return }
                ...
                project.setPadSamples(stems.prefix(Kit.pads.count).enumerated().map { (Kit.pads[$0].id, $1.audio, $1.name) },
                                      bank: destination.bank)   // one undo step, this bank only (F0)
    ...
    private func applyStretch(_ ratio: Double) {
        ...
        let destination = project.operationDestination()   // (#cross-1)
        Task.detached(priority: .userInitiated) {
            let stretched = wsolaStretch(src.data, ratio: ratio, sr: src.sr)
            await MainActor.run {
                analysisBusy = false
                guard destination.matches(project) else { flash("Project changed — stretch discarded"); return }
    ...
    private func runSampleFX(_ name: String, _ proc: @escaping @Sendable ([Float], Double) -> [Float]) {
        ...
        Task.detached(priority: .userInitiated) {
            ...
            await MainActor.run {
                analysisBusy = false
                guard destination.matches(project) else { flash("Project changed — \(name) discarded"); return }
    ...
    private func applyConvolution() {
        ...
        Task.detached(priority: .userInitiated) {
            ...
            await MainActor.run {
                analysisBusy = false
                guard destination.matches(project) else { flash("Project changed — reverb discarded"); return }
                guard !out.isEmpty else { flash("Couldn't process this sample"); return }
    ...
    private func applyPitch() {
        ...
        Task.detached(priority: .userInitiated) {
            ...
            await MainActor.run {
                analysisBusy = false
                guard destination.matches(project) else { flash("Project changed — pitch discarded"); return }
    ...
    private func applySpectral() {
        ...
        Task.detached(priority: .userInitiated) {
            ...
            await MainActor.run {
                analysisBusy = false
                guard destination.matches(project) else { flash("Project changed — spectral discarded"); return }
                guard !out.isEmpty else { flash("Couldn't process this sample"); return }
    ...
    private func toSynthKeys() {
        ...
        project.checkpoint("synthSrc", coalesce: false)   // undoable + dirty, like loadSampleSource (#undo-2)
        engine.sampleToSynth()
        project.synthPatch.source = "sample"
        project.synthPatch.bufferKind = "sampler"   // sentinel: "copy the sampler buffer" (see rebuildSynthSampleSource)
    ...
    private func tuneToKey() {
        ...
        Task.detached(priority: .userInitiated) {   // pitch detect off-main, like the sibling analyses (P2)
            ...
            await MainActor.run {
                analysisBusy = false
                guard destination.matches(project) else { flash("Project changed — tuning discarded"); return }
