...
/// ONE channel's live meter level. A dedicated ObservableObject per channel so a level change invalidates
/// only that strip's body — not the whole mixer mode (#METER-60FPS).
final class MeterCell: ObservableObject {
    @Published private(set) var value: Double = 0
    /// Publish only on a real change, so a decay tick that leaves a value where it was costs nothing.
    func set(_ v: Double) {
        let n = max(0, min(1, v))
        if n != value { value = n }
    }
...
/// Live meter levels for the mixer strips, one cell per channel. These used to be `@State` on
/// `MixerModeView` and reassigned on a 1/60 s timer, so every single tick re-evaluated the ENTIRE mode
/// body — TransportBar, MasterFXBar and every strip — on the same main queue as the 25 ms lookahead
/// scheduler. The mode now owns this container WITHOUT observing it (plain `@State`, not `@StateObject`:
/// the wrappers subscribe the owning view unconditionally); each strip observes only its own cell, the
/// tick is 30 Hz, and a tick that changes nothing publishes nothing. (#METER-60FPS)
final class MixMeters {
    static let decayPerTick = 0.05
    private var cells: [String: MeterCell] = [:]
    func cell(_ ch: String) -> MeterCell {
        if let c = cells[ch] { return c }
        let c = MeterCell(); cells[ch] = c; return c
    }
    func value(_ ch: String) -> Double { cell(ch).value }
    func set(_ ch: String, _ v: Double) { cell(ch).set(v) }
    /// Advance every live meter one frame. Silent channels are skipped entirely.
    func decay() {
        for c in cells.values where c.value > 0 {
            let nv = c.value - Self.decayPerTick
            c.set(nv <= 0.001 ? 0 : nv)
        }
    }
}
...
struct MixerModeView: View {
    ...
    @State private var meters = MixMeters()
    @State private var mixTab = 0   // 0 = Buses · 1 = Tracks (per-arrangement-track strips)
    /// 30 Hz is plenty for a decay meter and halves the main-thread work of the old 60 fps publish.
    ...
    @State private var meterTimer = Timer.publish(every: MixerModeView.meterInterval, on: .main, in: .common).autoconnect()
                        ...
                        ScrollView(.horizontal, showsIndicators: false) {   // narrow layouts (Split View) cramped the strips (#UXA11Y-03)
                            HStack(spacing: 12) {
                                ForEach(Kit.channels) { c in
                                    MixStrip(ch: c.id, name: c.name, color: c.color, meter: meters.cell(c.id), master: false)
                                }
                                MixStrip(ch: "melody", name: FDPalette.melodyName, color: FDPalette.melody, meter: meters.cell("melody"), master: false)
...
struct MixStrip: View {
    ...
    let name: String
    let color: Color
    /// This strip's own meter cell: a level change invalidates this strip's body only (#METER-60FPS).
    @ObservedObject var meter: MeterCell
    let master: Bool

    @State private var faderDrag = FDDragTrack()
    @State private var panDrag = FDDragTrack()
    // @GestureState resets on END *and* CANCEL — the only reliable "finger is gone" signal (#FADER-STALE).
    @GestureState private var faderTouching = false
    ...
    var body: some View {
        ...
        // Pinned like TrackStrip: a content-sized strip could reflow the whole row whenever a readout
        // ("−∞ dB", "CLIP", a longer name) changed width mid-drag.
        ...
        .onChange(of: faderTouching) { _, t in if !t { faderDrag.reset() } }   // end OR cancel (#FADER-STALE)
        .onChange(of: panTouching) { _, t in if !t { panDrag.reset() } }
    ...
    private var knob: some View {
        ...
        .frame(width: 46, height: 46)
        .contentShape(Circle())
        // highPriority: inside the strips' horizontal ScrollView a plain `.gesture` lost the touch to the
        // scroll pan as soon as the row overflowed (Split View / narrow window) — the knob went dead.
        .highPriorityGesture(DragGesture(minimumDistance: 0)
            .updating($panTouching) { _, s, _ in s = true }
            .onChanged { v in
                // A `minimumDistance: 0` gesture delivers a zero-translation frame on touch-down. That frame
                // must do NOTHING — no checkpoint (which would wipe the redo stack), no write (#MIX-UNDO).
                if FDDrag.moved(v.translation) { panDrag.moved = true }
                if panDrag.start == nil { panDrag.start = m.pan }
                guard FDDrag.shouldApply(moved: panDrag.moved, dragOpen: panDrag.open) else { return }
                if !panDrag.open {
                    panDrag.open = true
                    project.checkpoint("mix:\(ch)", coalesce: false)   // one undo step per drag
                }
                project.setMix(ch, emit: false) { $0.pan = max(-1, min(1, (panDrag.start ?? 0) + v.translation.width / 80)) }
            }
            .onEnded { _ in
                if panDrag.open { project.emitMix(ch) }   // one sync op per drag, not one per frame
                panDrag.reset()
            })
        ...
        GeometryReader { g in
            ZStack(alignment: .bottom) {
                ...
                RoundedRectangle(cornerRadius: 5)
                    ...
                    .frame(height: g.size.height * min(1, meter.value))
            }
        }
        .frame(width: 12)
    }
    ...
    private func fader(vol: Double) -> some View {
        GeometryReader { g in
            let H = g.size.height
            let frac = vol / AudioDefaults.maxChannelVol
            ZStack(alignment: .bottom) {
                ...
                .offset(y: -H * frac + 11)
                .highPriorityGesture(DragGesture(minimumDistance: 0)   // wins over the strips' scroll pan (see pan knob)
                    .updating($faderTouching) { _, s, _ in s = true }
                    .onChanged { v in
                        // Same rule as the pan knob: a zero-translation touch-down must not checkpoint (#MIX-UNDO).
                        if FDDrag.moved(v.translation) { faderDrag.moved = true }
                        if faderDrag.start == nil { faderDrag.start = m.vol }
                        guard FDDrag.shouldApply(moved: faderDrag.moved, dragOpen: faderDrag.open) else { return }
                        if !faderDrag.open {
                            faderDrag.open = true
                            project.checkpoint("mix:\(ch)", coalesce: false)
                        }
                        project.setMix(ch, emit: false) { $0.vol = max(0, min(AudioDefaults.maxChannelVol, (faderDrag.start ?? 0) - v.translation.height / H)) }
                    }
                    .onEnded { _ in
                        if faderDrag.open { project.emitMix(ch) }
                        faderDrag.reset()
                    })
            }
        ...
        .accessibilityAdjustableAction { dir in
            switch dir {
            case .increment: project.setMix(ch) { $0.vol = max(0, min(AudioDefaults.maxChannelVol, $0.vol + 0.05)) }
            case .decrement: project.setMix(ch) { $0.vol = max(0, min(AudioDefaults.maxChannelVol, $0.vol - 0.05)) }
            default: break
            }
        }
    }
...
struct MasterFXBar: View {
    ...
    var body: some View {
            ...
            HStack(alignment: .top, spacing: 16) {
                group("AUTO-MASTER", color: settings.accent) {
                    Button {
                        ...
                        // was also .frame(maxHeight: .infinity) — on a tall window that stretched this
                        // one button into a several-hundred-point empty box
        ...
        // hug the row's own height: the card used to absorb the window's spare vertical space, so on a
        // tall (portrait) window it became a mostly-empty half-screen panel
...
struct TrackStrip: View {
    ...
    @EnvironmentObject var settings: AppSettings
    let track: Track
    /// This strip's own meter cell (shared with its mixer channel); a level change invalidates this
    /// strip's body only (#METER-60FPS).
    @ObservedObject var meter: MeterCell
    // Same zero-distance rule as the channel strips: the first `minimumDistance: 0` frame arrives on
    // touch-down with no movement and must not checkpoint (#MIX-UNDO). Cleared on end OR cancel (#FADER-STALE).
    @State private var faderDrag = FDDragTrack()
    @State private var panDrag = FDDragTrack()
    @GestureState private var faderTouching = false
    @GestureState private var panTouching = false
    ...
    var body: some View {
        ...
        VStack(spacing: 9) {
            ...
            if track.isFrozen || track.isLinked || track.type == .bus {
                ...
                Button {
                    if !hasBus && !project.toggleTrackBus(track.id) { return }   // strip pool full — nothing to edit
                    showFX = true
                } label: {
    ...
    private var panKnob: some View {
        ...
        .highPriorityGesture(DragGesture(minimumDistance: 0)   // wins over the strips' scroll pan (see MixStrip)
            .updating($panTouching) { _, s, _ in s = true }
            .onChanged { v in
                // A zero-translation touch-down must not checkpoint (dirty + redo loss) (#MIX-UNDO).
                if FDDrag.moved(v.translation) { panDrag.moved = true }
                if panDrag.start == nil { panDrag.start = track.pan }
                guard FDDrag.shouldApply(moved: panDrag.moved, dragOpen: panDrag.open) else { return }
                if !panDrag.open {
                    panDrag.open = true
                    project.checkpoint("trackpan:\(track.id)", coalesce: false)   // one undo step per drag
                }
                project.setTrackPan(track.id, (panDrag.start ?? 0) + v.translation.width / 80)
            }
            .onEnded { _ in panDrag.reset() })
    ...
    private var fader: some View {
        GeometryReader { g in
            let H = g.size.height
            let frac = track.vol / AudioDefaults.maxTrackVol
            ZStack(alignment: .bottom) {
                RoundedRectangle(cornerRadius: 4).fill(settings.panel2.darker(0.22)).frame(width: 6)
                ...
                .highPriorityGesture(DragGesture(minimumDistance: 0)   // wins over the strips' scroll pan (see MixStrip)
                    ...
                    .onChanged { v in
                        // Same rule as the pan knob: a zero-translation touch-down must not checkpoint (#MIX-UNDO).
                        if FDDrag.moved(v.translation) { faderDrag.moved = true }
                        if faderDrag.start == nil { faderDrag.start = track.vol }
                        guard FDDrag.shouldApply(moved: faderDrag.moved, dragOpen: faderDrag.open) else { return }
                        if !faderDrag.open {
                            faderDrag.open = true
                            project.checkpoint("trackvol:\(track.id)", coalesce: false)
                        }
                        project.setTrackVol(track.id, (faderDrag.start ?? 0) - v.translation.height / H * AudioDefaults.maxTrackVol)
                    }
                    .onEnded { _ in faderDrag.reset() })
            }.frame(maxWidth: .infinity, alignment: .center)
        }.frame(width: 46)
