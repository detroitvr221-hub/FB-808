...
struct InfoTip: View {
    ...
    let detail: String
    var size: CGFloat = 16
    /// Tappable square around the badge. 44 pt is the HIG minimum for a standalone badge; a badge that
    /// sits ON another control (KnobView) must pass something close to `size`, or the invisible 44 pt box
    /// covers the control's centre and swallows its drags (FADER_AUDIT_2026-09-16 #1).
