...
struct TeacherModeView: View {
    ...
    @AppStorage("fd.studentName") private var studentName = ""   // every student used to join as "Student" (#sync-5)
    ...
    var body: some View {
        ...
        // Mirror the REAL session into the persisted `live` flag from the view root, not from the Live-tab
        // card (which is not in the hierarchy while another tab is showing) (round 2, sync-1).
        .onChange(of: session.connected) { _, c in if session.role == .host { classroom.live = c } }
        .onChange(of: session.role) { _, r in if r != .host { classroom.live = false } }
    }
        ...
        HStack(alignment: .top, spacing: 14) {
            VStack(spacing: 12) {
                VStack(spacing: 8) {
                    ...
                    Button { if session.role == .host { confirmEndLive = true } else { toggleLive() } } label: {
                        HStack(spacing: 8) {
                ...
                teCard("Live Connection") {   // Step 6: real Supabase Realtime status + student join
                    ...
                    if let err = session.lastError {   // was written by host()/follow() but rendered nowhere (#sync-3)
                        Text(err).font(FDFont.ui(12, .semibold)).foregroundStyle(settings.theme.miss)
                            .fixedSize(horizontal: false, vertical: true)
                    }
                    if session.role != .host {
                        HStack(spacing: 8) {
                            TextField("Your name", text: $studentName)
                                .font(FDFont.ui(13, .semibold)).autocorrectionDisabled()
                                .padding(.horizontal, 10).frame(height: 36).frame(maxWidth: 150)
                            ...
                            Button {
                                ...
                                let nm = studentName.trimmingCharacters(in: .whitespaces)
                                if !c.isEmpty && session.role != .host { session.follow(code: c, name: nm.isEmpty ? "Student" : String(nm.prefix(24))) }
                            } label: {
    ...
    private func pushToClass(_ label: String) {
        guard session.role == .host, session.connected else { flash("Not connected — \(label) was not sent"); return }
        flash("\(label) sent to \(onCount) students")
    }
    /// `classroom.live` follows the REAL session state (see the onChange below) instead of being flipped
    /// before the room exists — a failed create used to show "LIVE" forever (#sync-3).
    private func toggleLive() {
        if session.role == .host || classroom.live { session.leave(); classroom.live = false; flash("Live class ended") }
        else { session.host(title: TE_CLASS); flash("Starting live class…") }
    }
