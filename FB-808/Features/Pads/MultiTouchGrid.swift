...
struct MultiTouchGrid: UIViewRepresentable {
    ...
    let gap: CGFloat
    let padIDs: [String]
    var touchVelocity: Bool = false        // Settings → "Touch-sensitive pads": derive velocity from contact radius
    ...
    final class TouchView: UIView {
        ...
        private func velocity(_ t: UITouch) -> Double {
            ...
            // Contact radius is a noisy proxy (a fingertip reads ~15–25 pt and shifts with finger angle), so
            // it is opt-in, and the map stays inside a musical 0.6…1.0 (MAIN_SCREEN_AUDIT_2026-09-16 #5).
            guard cfg?.touchVelocity == true else { return 0.85 }
            let r = Double(t.majorRadius)
            if r > 0 { return max(0.6, min(1.0, 0.6 + (r - 8) / 50)) }   // ~8pt→0.6 … ~28pt→1.0
            return 0.85
        }
