...
struct PadGridView: View {
    ...
    var emptyIDs: Set<String> = []     // pads with nothing assigned (empty Bank C / D) — dimmed, dashed, "EMPTY"
    var touchVelocity: Bool = false    // Settings → "Touch-sensitive pads"
    ...
    var body: some View {
        GeometryReader { geo in
                ...
                ForEach(0..<4, id: \.self) { r in
                    HStack(spacing: gap) {
                        ForEach(0..<4, id: \.self) { c in
                            ...
                            PadView(pad: pad, showLabel: showLabels, badge: badges?[pad.id], muted: mutedIDs.contains(pad.id),
                                    empty: emptyIDs.contains(pad.id), onHit: { onHit($0, 0.85) }, onUp: onUp)
            ...
            .overlay(MultiTouchGrid(cols: 4, rows: 4, cell: cell, gap: gap,
                                    padIDs: pads.map(\.id), touchVelocity: touchVelocity, onDown: onHit, onUp: onUp))
...
struct PadView: View {
    ...
    var body: some View {
        ...
        ZStack {
            ...
            if empty {   // nothing assigned on this pad (empty Bank C / D): make that visible instead of a look-alike pad
                RoundedRectangle(cornerRadius: 19)
                    .stroke(th.inkFaint.opacity(0.7), style: StrokeStyle(lineWidth: 1.5, dash: [6, 5]))
                    .padding(3)
                Text("EMPTY").font(FDFont.mono(9, .bold)).tracking(1.4).foregroundStyle(th.inkFaint)
        ...
        .compositingGroup()
        .opacity(muted ? 0.6 : (empty ? 0.55 : 1))
        ...
        .accessibilityValue(Text(muted ? "Muted" : (empty ? "Empty" : "")))
    ...
    private var accessibilityHintText: String {
        ...
        if empty { return "Empty pad. Nothing assigned yet." }
