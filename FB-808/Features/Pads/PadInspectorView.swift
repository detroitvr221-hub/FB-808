...
struct PadInspectorView: View {
    ...
    /// The swatch list, exposed for the accessibility regression test.
    static let swatchColors = PI_COLORS
    /// VoiceOver name for a swatch. The 13 circles used to be unlabelled, so a blind user heard thirteen
    /// identical "button"s and could not tell which colour a pad was set to.
    nonisolated static func colorName(_ hex: String) -> String {
        switch hex.uppercased() {
        case "#FF5A3C": return "Coral"
        case "#FF7A1A": return "Orange"
        case "#FFC23C": return "Amber"
        case "#FFD84D": return "Yellow"
        case "#7AE582": return "Green"
        case "#33E0D4": return "Teal"
        case "#27C2E8": return "Sky"
        case "#5B8DEF": return "Blue"
        case "#6C7BFF": return "Indigo"
        case "#9B8CFF": return "Violet"
        case "#C77DFF": return "Purple"
        case "#E879F9": return "Magenta"
        case "#FF7AC6": return "Pink"
        default: return hex
        }
    }
    nonisolated static func swatchAccessibilityLabel(_ hex: String) -> String {
        "Pad colour \(colorName(hex))"
    }
    nonisolated static func swatchIsSelected(hex: String, current: String) -> Bool {
        hex.caseInsensitiveCompare(current) == .orderedSame
    }
    ...
    private var modal: some View {
        VStack(spacing: 0) {
            header
            MPCCoachStrip(section: "Pad Editor").padding(.horizontal, 16).padding(.top, 12)   // Settings → MPC Coach
    ...
    private var leftColumn: some View {
        VStack(spacing: 12) {
            ...
            card("Color") {
                let current = pp.colorHex ?? colorHexOf(pad.color)
                // 4 columns in the 264 pt column → ≥44 pt cells, so the hit target meets the HIG minimum
                // instead of the old ~28×24 pt circles that adjacent-mistapped (#PAD-SWATCH).
                ...
                LazyVGrid(columns: cols, spacing: 7) {
                    ForEach(PI_COLORS, id: \.self) { hex in
                        let isSelected = Self.swatchIsSelected(hex: hex, current: current)
                        Button { up { $0.colorHex = hex } } label: {
                            Circle().fill(Color(hex: hex)).frame(width: 24, height: 24)
                                ...
                                // Non-colour cue: a checkmark marks the selected swatch, so selection isn't
                                // conveyed by the thin white ring alone.
                                .overlay(
                            ...
                            .accessibilityLabel(Text(Self.swatchAccessibilityLabel(hex)))
            ...
            ForEach(Array(pp.layers.enumerated()), id: \.element.id) { (i, ly) in
                HStack(spacing: 8) {
                    ...
                    Button { up { $0.layers.remove(at: i) } } label: {
                        ...
                        .accessibilityLabel(Text("Remove layer \(i + 1)"))
                }
            }
        }
    }
