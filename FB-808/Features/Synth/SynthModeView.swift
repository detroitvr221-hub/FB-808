...
struct SynthModeView: View {
    ...
    /// Coalesce per PARAMETER: one shared "synth" key merged cutoff-then-resonance (and edits to two different
    /// parts) into a single undo step (#undo-5).
    private func up<T>(_ kp: WritableKeyPath<SynthPatch, T>, _ v: T) { project.checkpoint("synth:\(project.activePart):\(kp)"); project.editPatch[keyPath: kp] = v }
        ...
        VStack(spacing: 12) {
            ...
            GeometryReader { g in
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    // Half the height instead of a flat 360: on a tall (portrait) window that flat cap
                    // cut the Instrument Layers buttons in half; on a landscape window it lands ~360,
                    // so the keyboard below keeps the room it had.
                    .frame(maxHeight: min(460, max(280, g.size.height * 0.5)))
                    playArea
                }
    ...
    private var presetBar: some View {
        HStack(spacing: 8) {
            ...
            presetArrow("chevron.left", "Previous preset") { cyclePreset(-1) }
    ...
    /// VoiceOver has for prev/next and for the one-tap "save this patch" star (finding 66).
    private func presetArrow(_ icon: String, _ label: String, action: @escaping () -> Void) -> some View {
            ...
            VStack(spacing: 0) {
                HStack {
                    ...
                    Button { showBrowser = false } label: {
                        ...
                        .accessibilityLabel(Text("Close sound browser"))
                }
        ...
        PanelCard(title: "Play Assist") {
            ...
            if project.arpMode != "off" {
                assistSeg([("1/8", "⅛"), ("1/16", "1/16"), ("1/16T", "16T"), ("1/32", "1/32")], sel: project.arpRate) {
                    project.setArpRate($0); if !project.assistHeld.isEmpty { project.restartArp() }
                }
                HStack(spacing: 5) {
                    Text("OCT").font(FDFont.mono(9, .bold)).foregroundStyle(settings.inkFaint)
                    ForEach(1...3, id: \.self) { o in
                        Button { project.setArpOct(o); if !project.assistHeld.isEmpty { project.restartArp() } } label: {
        ...
        Button(action: action) {
            HStack(spacing: 4) {
                ...
                Text(label).font(FDFont.ui(12.5, .semibold))
                    .lineLimit(1).minimumScaleFactor(0.8)   // "Chords" was wrapping to "Chor/ds"
            }
    ...
    private var keysSide: some View {
        VStack(spacing: 8) {
            ...
            sideToggle(project.scaleLock ? "lock.fill" : "lock.open", "Scale Lock", on: project.scaleLock) { project.setScaleLock(!project.scaleLock) }
        ...
        HStack(spacing: 8) {
            HStack(spacing: 4) {
                ...
                ForEach([(1, "1/16"), (2, "1/8"), (4, "1/4"), (8, "1/2"), (16, "1/1")], id: \.0) { (len, lbl) in
                    Button { project.setRollLen(len) } label: {
...
struct KnobView: View {
    ...
    @GestureState private var touching = false   // resets on end OR cancel — see FDDragTrack (#FADER-STALE)
    ...
    var body: some View {
        VStack(spacing: 4) {
            ...
            .contentShape(Circle())
            // highPriority: in an overflowing horizontal ScrollView (the Mixer's master FX bar in portrait /
            // Split View) a plain `.gesture` lost every touch to the scroll pan and the knob was dead
            // (FADER_AUDIT_2026-09-16 #1). The knob owns the touch; the bar scrolls from the gaps.
        ...
        .frame(width: size + 16)
        .onChange(of: touching) { _, t in if !t { dragStart = nil; moved = false } }   // end OR cancel (#FADER-STALE)
        .overlay(alignment: .topTrailing) {
            // hitSize ≈ badge: the default 44 pt hit box reached past the knob's centre and ate every drag
            // that started there — the Mixer FX knobs read as "dead" (FADER_AUDIT_2026-09-16 #1).
            if let info { InfoTip(term: label, detail: info, size: 15, hitSize: 22).offset(x: 5, y: -3) }
        }
        .accessibilityElement(children: .ignore)
        .accessibilityLabel(Text(label))
...
struct SynthRoll: View {
    ...
    private func rowDrag(_ pitch: Int, _ x: CGFloat, _ width: CGFloat) {
        ...
        // This roll always draws 16 columns, but the bar can be shorter (16/12/8): a fresh touch on a
        // column Transport never visits must not plant a note there — the drum grids already refuse
        // out-of-bar columns (finding 28). An edit already in flight still runs, and the write paths
        // clamp it to the bar.
        guard cell < max(1, project.barSteps) || drawDrag != nil || moveDrag != nil else { return }
