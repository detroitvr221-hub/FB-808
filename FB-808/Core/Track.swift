...
/// The live (classic) sources a track reproduces from its own content, so the classic playback paths must
/// NOT trigger them a second time. Deliberately decoupled from `isLinked`: freezing a linked track clears
/// its link, yet the track still carries a copy of that source — so it must keep owning it, or the source
/// is heard twice (live, in the WAV bounce and in the .mid). (#15)
struct TrackOwnership {
    var rows = Set<String>()
    var allLanes = false          // the link owned every lane row (LinkRef.rows == nil)
    var leadMelody = false
    var partIDs = Set<String>()

    var isEmpty: Bool { rows.isEmpty && !allLanes && !leadMelody && partIDs.isEmpty }

    init() {}
    init(link: LinkRef) {
        switch link.kind {
        case .lanes:  if let r = link.rows { rows.formUnion(r) } else { allLanes = true }
        case .melody: leadMelody = true
        case .part:   if link.partID == nil || link.partID == "lead" { leadMelody = true } else if let pid = link.partID { partIDs.insert(pid) }
        case .sequenceLanes, .sequenceMelody: break
        }
    }
    mutating func formUnion(_ o: TrackOwnership) {
        rows.formUnion(o.rows); allLanes = allLanes || o.allLanes
        leadMelody = leadMelody || o.leadMelody; partIDs.formUnion(o.partIDs)
    }
}
...
extension Project {
    ...
    func addTrack(_ type: TrackType) -> String {
        ...
        if type == .bus && busOrder.count >= AudioDefaults.maxBuses { return "" }   // strip pool full (#cross-4)
    ...
    func setTrackVol(_ id: String, _ v: Double) {
        guard let i = tracks.firstIndex(where: { $0.id == id }) else { return }
        let nv = max(0, min(AudioDefaults.maxTrackVol, v))
        guard tracks[i].vol != nv else { return }   // no-op touch → no checkpoint, no dirty, no redo loss (#MIX-UNDO)
    ...
    func setTrackPan(_ id: String, _ p: Double) {
        guard let i = tracks.firstIndex(where: { $0.id == id }) else { return }
        let np = max(-1, min(1, p))
        guard tracks[i].pan != np else { return }   // idempotent (#MIX-UNDO)
    ...
    /// Returns false when the engine's strip pool is full: the DSP allocates at most `AudioDefaults.maxBuses`
    /// insert-FX buses, and a track past that silently rendered through bus 0 with no inserts (#cross-4).
    @discardableResult
    func toggleTrackBus(_ id: String) -> Bool {
        guard let i = tracks.firstIndex(where: { $0.id == id }) else { return false }
        if !tracks[i].ownsBus && busOrder.count >= AudioDefaults.maxBuses { return false }
    ...
    func unfreezeTrack(_ id: String) {
        ...
        checkpoint("unfreeze:\(id)", coalesce: false)   // ONE undo step for the whole unfreeze (#undo-7)
        // Every bounce/take on the track goes, or live synthesis doubles the leftover clip (round 3, undo-3).
        // WAVs are kept on disk like removeAudioClip does — undo re-reads them; the sweep reclaims later.
        audioClips.removeAll { $0.track == id }
    ...
    /// See `TrackOwnership`. A LIVE-LINKED track owns whatever its link points at; a FROZEN copy owns what
    /// it captured (derived from the copy itself, which also repairs projects saved before this rule
    /// existed); a frozen-to-audio track still owns its link's sources because its bounce clip plays
    /// instead of the live synthesis. (#15)
    func trackOwnership(_ track: Track) -> TrackOwnership {
        var o = TrackOwnership()
        if let link = track.source.link, track.isLinked || track.frozenToAudio { o.formUnion(TrackOwnership(link: link)) }
        if !track.isLinked {
            switch track.type {
            case .drumPattern: if let lanes = track.source.lanes { o.rows.formUnion(lanes.keys) }
            case .synthPart:
                if let pid = track.source.partID, !pid.isEmpty {
                    if pid == "lead" { o.leadMelody = true } else { o.partIDs.insert(pid) }
                }
            default: break
            }
        }
