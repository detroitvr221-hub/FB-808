...
final class AppSettings: ObservableObject {
    ...
    /// Strike velocity from finger contact radius (no pressure sensor on iPad). Off by default: the radius
    /// map made ordinary fingertip hits land ~0.55–0.8 and vary with finger angle (MAIN_SCREEN_AUDIT #5).
    @Published var padTouchVelocity: Bool { didSet { store.set(padTouchVelocity, forKey: "fd.padTouchVelocity") } }
    ...
    @Published var haptics: Bool         { didSet { store.set(haptics, forKey: "fd.haptics") } }            // "feel the beat" — beat pulse while playing AND pad-hit feedback (#69)
    ...
    init() {
        ...
        padTouchVelocity = store.object(forKey: "fd.padTouchVelocity") as? Bool ?? false
    ...
    func resetToDefaults() {
        ...
        padLabels = true; glow = 1.0; mpcCoach = false; padTouchVelocity = false
    ...
    /// The audio-quality / polyphony / limiter values as the AUv3 must render them (finding 55).
    /// `RootView.applyAudio` publishes this into the App Group on launch and on every change.
    var sharedAudioSettings: SharedAudioSettings {
        SharedAudioSettings(hqInterp: hqInterp, equalPowerPan: equalPowerPan, bandlimitedOsc: bandlimitedOsc,
                            polyphony: polyphony, limiterOn: limiterOn, limiterCeilingDb: limiterCeilingDb)
    }
}
